...
# 排除2024年后上市的次新股
df = df[df['代码'].str.startswith(('000', '001', '002', '300')) == False]
# 主板和中小板保留代码开头为 600/601/603 的老股票
...
# PE筛选：0-25
df_pe = df_clean[(df_clean['市盈率-动态'] > 0) & (df_clean['市盈率-动态'] < 25)]
print(f"    PE<25: {len(df_pe)} 只")

# PB筛选：0-3
df_pb = df_pe[df_pe['市净率'] < 3]
print(f"    PB<3: {len(df_pb)} 只")

# 市值筛选：50-2000亿
df_size = df_pb[(df_pb['流通市值'] > 50) & (df_pb['流通市值'] < 2000)]
print(f"    市值50-2000亿: {len(df_size)} 只")

# 第4步：跌幅筛选（找超跌的）
print("\n[4/5] 超跌筛选...")

# 重点找年初至今跌幅较大的（被错杀的）
df_oversold = df_size[df_size['年初至今涨跌幅'] < 0]
...
for i, (_, row) in enumerate(sample.iterrows(), 1):
    ...
    try:
        ...
        if fin is not None and len(fin) >= 4:
            ...
            # 营收增长
            if '营业总收入(元)' in fin.columns:
                rev_vals = fin['营业总收入(元)'].dropna().tolist()
                if len(rev_vals) >= 2 and rev_vals[1] > 0:
                    revenue_growth = (rev_vals[0] - rev_vals[1]) / rev_vals[1]
    ...
    if revenue_growth > 0.1: score += 15
    ...
    print(f"ROE={roe:.1f}%, 利润增长={profit_growth:+.1%}, 得分={score}")
...
print("\n筛选标准:")
print("  1. PE < 25，PB < 3")
print("  2. 排除：金融、地产、周期、出口、光伏、锂电")
print("  3. 市值：50-2000亿")
print("  4. 年初至今下跌（被错杀）")
print("  5. ROE > 0，盈利稳定/增长")
print()
...
for i, r in enumerate(results[:20], 1):
    ...
    print(f"    年初至今: {r['ytd']:+.1f}% | 市值: {r['mkt_cap']/10000:.1f}亿")
...
for i, r in enumerate(results[:5], 1):
    ...
    if r['pb'] < 1:
        ...
        pb_grade = "较高"
    print(f"  PB = {r['pb']:.2f} ({pb_grade})")

    # 业绩分析
    print(f"\n【业绩】")
    if r['roe'] > 0:
        print(f"  ROE = {r['roe']:.1f}%")
        print(f"  趋势: {'上升' if r['roe_change'] > 0.1 else '稳定' if r['roe_change'] > -0.1 else '下滑'}")
    else:
        print(f"  ROE = N/A")
    ...
    if r['revenue_growth'] != 0:
        print(f"  营收增长: {r['revenue_growth']:+.1%}")
    ...
    if r['ytd'] < -40:
        ...
        reason = "年初至今跌幅超30%，被市场过度悲观"
    elif r['ytd'] < -20:
        reason = "年初至今跌幅20%以上，可能被错杀"
    elif r['change'] < -5:
        reason = "今日大跌，可能受战争恐慌拖累"
    else:
        reason = "估值低+盈利稳定，但市场未认可"
    print(f"  {reason}")

    # 翻倍潜力
    print(f"\n【翻倍潜力】")
    potential_score = 0
    if r['pe'] < 10: potential_score += 30
    elif r['pe'] < 15: potential_score += 20

    if r['roe'] > 15: potential_score += 25
    elif r['roe'] > 10: potential_score += 20
    elif r['roe'] > 5: potential_score += 10

    if r['profit_growth'] > 0.2: potential_score += 20
    elif r['profit_growth'] > 0: potential_score += 10

    if r['ytd'] < -30: potential_score += 15
    elif r['ytd'] < -20: potential_score += 10
