...
trade = {
    ...
    'action': 'SELL',
    'sell_price': 0.99,
    'quantity_pct': 4.265,  # 原仓位8.53%的一半
    'cost': 1.07,
    'date': '2026-03-26',
    'reason': '接近止损线 - 亏损-7.01%',
    'strategy': '熊市防御策略',
    'result': 'loss',
    'profit_pct': (0.99 - 1.07) / 1.07 * 100,
}
...
if -8 < trade['profit_pct'] < -6:
    print(f"  分析: 亏损-7.48%，接近止损线（-8%）")
    print(f"  原因: 计算机板块在熊市中表现弱势")
    print(f"  决策: 提前减仓是明智的，保护本金")
    print(f"  评价: 风险管理一致，执行纪律好")

print(f"\n【改进建议】")
print(f"  1. 计算机是高风险板块，熊市不适合重仓")
print(f"  2. 虽然十五五规划支持科技，但短期技术面弱")
print(f"  3. 减仓是对的，等待大盘企稳再加仓")
print(f"  4. 剩余4.265%可继续持有，等待反弹")

print(f"\n【策略调整】")
print(f"  - 剩余4.265%仓位: 继续持有，等待反弹")
print(f"  - 释放的现金: 4.265%可用于防御性建仓")
print(f"  - 建议: 用释放的现金买入银行/医药")
print(f"  - 触发条件: 如果跌破0.98元，继续减仓")

print(f"\n【账户影响】")
print(f"  - 总仓位: 76.99% → 72.725%（减少4.265%）")
print(f"  - 现金增加: 约4.27万元（假设总资产100万）")
...
print(f"  → 策略一致：熊市优先保护本金，系统性减仓")

print(f"\n【风险提示】")
print(f"  - 你在快速减仓高风险资产（有色、科创、计算机）")
print(f"  - 这是正确的熊市策略")
print(f"  - 继续这个节奏，目标降到50%以下")
